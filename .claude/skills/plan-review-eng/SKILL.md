...
- **Complexity awareness:** What's the hardest technical problem? Does the user even know where the complexity lives?
- **Build vs buy:** What's built from scratch vs using existing tools? The decision reveals engineering judgment.
- **Failure modes:** Where does this break first under real load? "It won't break" is the worst answer.
- **Abstraction quality:** Are the abstractions earned or premature? Does each layer justify its existence?

The team lead will only ask questions where the answer ISN'T already in the project docs. If ARCHITECTURE.md already describes the stack choices and known failure modes, no questions are needed — you review from the code.
